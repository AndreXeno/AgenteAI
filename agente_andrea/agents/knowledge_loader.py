...
def extract_text_from_file(file_path):
    """Estrae testo da PDF, DOCX o TXT con gestione errori e log."""
    ext = os.path.splitext(file_path)[1].lower()
    text = ""
    ...
    try:
        if ext == ".pdf":
            ...
            # Estrazione testo da TXT
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                text = f.read()
        else:
            print(f"[WARN] Tipo di file non supportato: {file_path}")
            return ""

        # Tronca se troppo lungo
        if len(text) > MAX_CHARS_PER_FILE:
            print(f"[LOG] 📏 Testo troncato da {len(text)} a {MAX_CHARS_PER_FILE} caratteri per {file_path}")
            text = text[:MAX_CHARS_PER_FILE]

        print(f"[LOG] ✅ Testo estratto da {file_path} ({len(text)} caratteri).")
        return text.strip()

    except Exception as e:
        print(f"[ERROR] ❌ Errore durante la lettura di {file_path}: {e}")
        return ""


# ======================================
# 📦 CARICAMENTO COMPLETO DELLA CONOSCENZA
# ======================================

def load_all_knowledge():
    """Legge tutti i file nella cartella 'knowledge/' e restituisce un unico blocco di testo."""
    if not os.path.exists(KNOWLEDGE_DIR):
        os.makedirs(KNOWLEDGE_DIR)
        print("[LOG] 📂 Cartella 'knowledge' creata.")
        return "Nessuna conoscenza caricata (cartella vuota)."
    ...
    if not all_text:
        print("[LOG] ℹ️ Nessun documento valido trovato nella cartella 'knowledge/'.")
        return "Nessun documento trovato nella cartella 'knowledge/'."

    print(f"[LOG] ✅ {len(all_text)} documenti caricati correttamente.")
    return "\n".join(all_text)
...
def query_knowledge(user_query: str, k: int = 3):
    """
    Cerca nei documenti caricati nella cartella 'knowledge' i paragrafi più pertinenti
    rispetto alla query dell’utente. Restituisce un elenco dei frammenti più rilevanti.
    """
    if not os.path.exists(KNOWLEDGE_DIR):
        print("[WARN] ⚠️ Cartella 'knowledge' non trovata.")
        return []
    ...
    print(f"[LOG] 🔎 Trovati {len(top_fragments)} frammenti rilevanti per la query: '{user_query}'")
    return [f"📘 {filename}:\n{para}" for _, filename, para in top_fragments]